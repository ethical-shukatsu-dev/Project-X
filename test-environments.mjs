...
console.log(`Production environment file (.env.production): ${hasProductionEnv ? '✅ Found' : '❌ Not found'}`);
...
async function testEnvironments() {
  ...
  console.log(`Development environment (.env.local): ${devConfigured ? '✅ Configured' : '❌ Not configured'}`);
  console.log(`Production environment (.env.production): ${prodConfigured ? '✅ Configured' : '❌ Not configured'}`);
