...
function useStepTracking(totalSteps: number) {
  ...
  const trackStepChange = (newStepIndex: number, stepId: string) => {
    ...
    if (previousStepId && previousStepIndex !== newStepIndex) {
      ...
      trackSurveyStepCompleted(previousStepIndex, previousStepId, totalSteps).catch((err) =>
        console.error('Error tracking step completion:', err)
      );
      ...
      if (currentStepIdRef.current && !isFirstRender) {
        ...
        if (navigator.sendBeacon) {
          ...
          trackSurveyStepAbandoned(
            ...
            timeSpent,
            'page_exit'
          ).catch((err) => console.error('Error tracking step abandonment during unload:', err));
        }
      }
    };
    ...
    return () => {
      ...
      if (currentStepIdRef.current && !isFirstRender) {
        ...
        trackSurveyStepAbandoned(
          ...
          timeSpent,
          'component_unmounted'
        ).catch((err) => console.error('Error tracking step abandonment on unmount:', err));
      }
    };
  }, [totalSteps]);
...
export default function ValuesQuestionnaire({
  ...
  const { trackStepChange, setAnswerSelected, initializeFirstStep } = useStepTracking(
    QUESTIONS.length
  );
  ...
  const handleSubmit = async () => {
    try {
      ...
      const url = createUrlWithParams(`/${lng}/recommendations`, searchParamsObj, {
        exclude: ['userId', 'type'],
        include: {
          userId: data.userId,
        },
    ...
    return (
      <Card className="w-full max-w-md mx-auto bg-gradient-to-b from-white/5 to-white/[0.02] backdrop-blur-sm border border-white/10 shadow-xl hover:shadow-blue-500/10 transition-all duration-300">
        <CardHeader className="flex flex-col items-center justify-center py-8">
          ...
          <CardTitle className="text-xl text-gray-300 animate-pulse">Loading...</CardTitle>
        </CardHeader>
      </Card>
    );
  }
            ...
            >
              {question.options.map((option: { value: string; labelKey: string }) => (
                <div key={option.value} className="flex items-center space-x-2">
                  ...
                  />
                  <Label htmlFor={option.value} className="text-base text-gray-300">
                    {t(option.labelKey)}
                  </Label>
                </div>
              ))}
