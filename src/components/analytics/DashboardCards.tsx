...
export function RefreshButton({ onClick }: RefreshButtonProps) {
  return (
    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={onClick ? onClick : () => {}}>
...
export function MetricCard({ title, value, description, onRefresh }: MetricCardProps) {
  return (
    <Card>
      <CardHeader>
...
export function ABTestComparison({ title, description, data, onRefresh }: ABTestComparisonProps) {
  ...
  const renderDifferenceIndicator = (value: string) => {
    ...
    return (
      <Badge variant={positive ? 'default' : 'destructive'} className="ml-2 flex items-center">
        {positive ? (
    ...
    <Card className="col-span-1 md:col-span-3">
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  ...
                  <Progress value={parseInt(data.anonymous.completionRate)} className="h-2" />
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-1">
                  ...
                  <Progress value={parseInt(data.anonymous.conversionRate)} className="h-2" />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-muted/50">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">Non-Anonymous Users</CardTitle>
              <CardDescription className="text-xs">
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  ...
                  <Progress value={parseInt(data.nonAnonymous.completionRate)} className="h-2" />
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span>Conversion Rate</span>
                  ...
                  <Progress value={parseInt(data.nonAnonymous.conversionRate)} className="h-2" />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-muted/50">
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-xs">Anonymous vs. Non-Anonymous</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6 py-2">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    {isPositive(data.difference.completionRate) ||
                    data.difference.completionRate === '0%'
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    {isPositive(data.difference.conversionRate) ||
                    data.difference.conversionRate === '0%'
