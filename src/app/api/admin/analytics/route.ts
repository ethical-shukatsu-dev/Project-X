...
export async function GET(
  ...
  try {
    ...
    const surveyStartRate = uniqueVisits > 0 ? Math.round((uniqueStarts / uniqueVisits) * 100) : 0;

    const surveyCompletionRate =
      uniqueStarts > 0 ? Math.round((uniqueCompletions / uniqueStarts) * 100) : 0;

    const overallConversionRate =
      uniqueVisits > 0 ? Math.round((uniqueCompletions / uniqueVisits) * 100) : 0;
    ...
    const signupMetrics = {
      ...
      totalSignups:
        (uniqueCounts?.signups?.email_signups || 0) + (uniqueCounts?.signups?.google_signups || 0),
    ...
    const surveyStepMetrics = (uniqueCounts?.survey_steps || [])
      .map((step: SurveyStep) => ({
        ...
        percentage:
          uniqueStarts > 0 ? `${Math.round((step.unique_users / uniqueStarts) * 100)}%` : '0%',
    ...
    const dropoffMetrics = (uniqueCounts?.survey_dropoffs || [])
      .map((step: SurveyDropoff) => ({
        ...
        completed:
          uniqueCounts?.survey_steps?.find((s: SurveyStep) => s.step_id === step.step_id)
            ?.unique_users || 0,
    ...
    const anonymousMetrics = {
      ...
      percentage: `${Math.round(
        ((uniqueCounts?.anonymous_users?.total_unique_users || 0) / uniqueVisits) * 100
      )}%`,
      completionRate: `${Math.round(
      ...
      conversionRate: `${Math.round(
        ((uniqueCounts?.anonymous_users?.completed_surveys || 0) / uniqueVisits) * 100
      )}%`,
    };
    ...
    const dialogCloseConversionRate =
      uniqueVisits > 0 ? Math.round((uniqueDialogCloses / uniqueVisits) * 100) : 0;
    ...
    const nonAnonymousCompletions =
      uniqueCompletions - (uniqueCounts?.anonymous_users?.completed_surveys || 0);
...
async function getSpecificMetric(
  ...
  try {
    ...
    if (uniqueError) {
      console.error('Error fetching unique counts for specific metric:', uniqueError);
      return NextResponse.json({ error: 'Failed to fetch unique counts' }, { status: 500 });
    }
    ...
    return NextResponse.json(
      {
        error: 'An unexpected error occurred while fetching the specific metric',
      },
      { status: 500 }
    );
  }
