...
async function deduplicateCompaniesInteractive() {
  ...
  try {
    ...
    const duplicateGroups = Object.entries(companyGroups)
    ...
    for (const [companyName, group] of duplicateGroups) {
      ...
      // Get user choice
      const answer = await prompt('Enter the number of the company to keep (or "auto" to use automatic selection): ');
